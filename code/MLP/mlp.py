...
def print_model_summaries(histories):
    print("\nModel comparison:")
    print()
    print(
        f"{'Model':<24} {'Best Train Acc':<15} {'Best Val Acc':<15} {'Best Val Loss':<15}"
    )
    print("-" * 72)
...
def train(X_train, y_train):
    ...
    print_model_summaries(histories)
    best_model_name, best_model = select_best_model(models, histories)
    best_val_acc = max(histories[best_model_name]["val_acc"])
    best_val_loss = min(histories[best_model_name]["val_loss"])
    print(
        f"Selected MLP model: {best_model_name} | "
