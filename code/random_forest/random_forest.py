...
MAX_DEPTH = 8  # Max depth of the decision trees
MIN_SPLITTING_SIZE = 20  # Sample count below which the tree should decide for the plurality class
DECISION_THRESHOLD = 0.92  # If a tree node has above this percentage of samples in one class, the tree decides for that class
...
def best_avg_entropy(data: pd.DataFrame, target: np.ndarray, feature_name: str):
    ...
    for thresh in thresh_range:
        ...
        avg_entropies.append(entropy(under_classes) * (len(under_classes) / len(target)) + entropy(over_classes) * (
                    len(over_classes) / len(target)))
    np_entropies = np.array(avg_entropies)
    return np.min(np_entropies), thresh_range[np.argmin(np_entropies)]


def construct_node(data: pd.DataFrame, target: np.ndarray, features, parent_level: int, max_depth: int = MAX_DEPTH,
                   min_size: int = MIN_SPLITTING_SIZE, decision_thresh: float = DECISION_THRESHOLD):
    """
    ...
    this_node.under = construct_node(data[under], target[under], features, parent_level + 1, max_depth, min_size,
                                     decision_thresh)
    this_node.over = construct_node(data[over], target[over], features, parent_level+1, max_depth, min_size,
                                    decision_thresh)
    return this_node
...
def train(x: pd.DataFrame, y: pd.DataFrame, n = NUM_TREES, max_depth = MAX_DEPTH, min_size = MIN_SPLITTING_SIZE, decision_thresh = DECISION_THRESHOLD):
    """
    ...
    for _ in range(n):
        ...
        trees.append(construct_node(xb, yb, features, 0, max_depth, min_size, decision_thresh))
